...
- Run one loop supervised: `bash bin/run-loop.sh <loop> <config>` where `<loop>` ∈ orchestrator | e2e-user | fixer | e2e-tester | pr-verifier | manager
- Queue funnel table (mechanical): `bash bin/funnel.sh <config>` — every open issue/PR → state → next actor (orphans flagged)
...
**Engine/adapter split — the core invariant.** Everything in this repo is generic. Every project-specific fact lives in the target project's `ops/loop.config.sh`. Never hardcode a project fact into `bin/`, `roles/`, or `contracts/`; add a variable to `config/loop.config.example.sh` instead.

**Markdown is load-bearing.** `roles/<role>/<loop>.md` is piped verbatim as the loop's prompt (`run-loop.sh` resolves `roles/*/<loop>.md` by glob — role folders are organization, loop basenames are the identifiers, and prompt-lint enforces their uniqueness). Each role folder carries a `CHARTER.md` (mission/authority/workspace contract). Dispatcher prompts paste `roles/*/subagents/*.md` into fresh subagents, and all prompts tell agents to read `contracts/*.md` at runtime via `$ILOOP_ROOT`. So renaming/moving these files, or renaming a config variable (`GATES`, `NOGO_PATHS`, `COOLING_HOURS`, `DEPLOY_*`, `E2E_*`, …), breaks prompts that reference them by literal path/name — grep across bin/, roles/, contracts/, config/, and docs/ before renaming anything. The `Return (exact shape)` blocks in subagent prompts are interfaces parsed by their dispatchers, and `SCHEDULE` loop names must match a `roles/*/<loop>.md` prompt.

**Two safety layers that must stay in sync.** Mechanical floors live in `bin/run-loop.sh` — singleton lock, `ops/PAUSE` kill switch, 3-strikes breaker, hard timeout, preflight, and post-run: no-go revert, self-accept guard, queue lint, stale loop-branch prune, diff accounting, digest guarantee, `> NOTIFY:` fan-out, retention caps. Behavioral rules live in `contracts/` and the agent prompts. Several rules exist in both layers: the bash queue lint mirrors `contracts/issue-format.md`, the self-accept guard mirrors `contracts/write-policy.md`'s human-only labels, the NOTIFY fan-out mirrors `contracts/digest-format.md`. When changing a contract, check whether a floor in `run-loop.sh` implements the same rule, and vice versa. Design law #1: anything money- or safety-shaped belongs in bash, not only in prompt text.
...
**Label vocabulary is a contract.** `templates/labels.sh`, `contracts/queue-state-machine.md`, the bash queue lint in `bin/run-loop.sh`, `bin/funnel.sh`, `bin/scorecard.sh`, and the agent prompts all encode the same labels (`loop-filed`, `accepted`, `action:*`, `component:*`, `bug`, `loop-pr`, `🙋 needs-your-decision`, `changes-requested`) — change them everywhere or nowhere.
...
**Self-hosting rails.** When this repo is its own target, the loop may edit `bin/`, `roles/`, `contracts/`, `docs/`, `templates/`, `tests/` via PRs, but never `selfhost/` (the promotion gate + e2e rubric), `ops/DIRECTION.md`, `ops/loop.config.sh`, or `.github/` — and it executes from a promoted live checkout, not this tree (`docs/SELF-HOSTING.md`).
